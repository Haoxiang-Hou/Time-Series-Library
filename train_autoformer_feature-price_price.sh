...
data=CMEES
dataset_class=SelectPriceFeaturesSeriesDataset
features=M
enc_in=16
dec_in=16
c_out=16
        ...
        for seq_len in 96; do
            ...
            for pred_len in 96 ; do
                for learning_rate in 5e-3 1e-3 5e-4 1e-4; do
                    ...
                    echo python -u run.py --task_name $task_name --is_training $is_training --model $model_name --data $data --dataset_class $dataset_class --features $features --enc_in $enc_in --dec_in $dec_in --c_out $c_out --target_size $target_size --des "$des" --itr $itr --freq_per_second $freq_per_second --seq_len $seq_len --label_len $label_len --pred_len $pred_len --d_model $d_model --d_ff $d_ff --n_heads $n_heads --e_layers $e_layers --d_layers $d_layers --factor $factor --moving_avg $moving_avg --model_id $model_id --setting $exp_name --train_epochs $train_epochs --train_rolling_window_stride $train_rolling_window_stride --dev_rolling_window_stride $dev_rolling_window_stride --learning_rate $learning_rate --batch_size $batch_size
                    # srun -p a10,4090 --gres=gpu:1 -c 8 --mem-per-cpu=10G --qos qnormal 
                    # python -u run.py --task_name $task_name --is_training $is_training --model $model_name --data $data --dataset_class $dataset_class --features $features --enc_in $enc_in --dec_in $dec_in --c_out $c_out --target_size $target_size --des "$des" --itr $itr --freq_per_second $freq_per_second --seq_len $seq_len --label_len $label_len --pred_len $pred_len --d_model $d_model --d_ff $d_ff --n_heads $n_heads --e_layers $e_layers --d_layers $d_layers --factor $factor --moving_avg $moving_avg --model_id $model_id --setting $exp_name --train_epochs $train_epochs --train_rolling_window_stride $train_rolling_window_stride --dev_rolling_window_stride $dev_rolling_window_stride --learning_rate $learning_rate --batch_size $batch_size &
                done
            done
        done
    done
